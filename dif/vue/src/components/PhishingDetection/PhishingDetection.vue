...
const normalizedResult = computed(() => {
    ...
    return {
        ...
        features: (() => {
            // 兼容多种后端格式
            if (detection.token_attribution) {
                // 旧格式：直接的 token_attribution 数组
                return detection.token_attribution.map(item => ({
                    name: item.token || item.piece,
                    score: Math.abs(item.score),
                    description: `贡献分数 ${item.score.toFixed(4)}`
                }))
            }
            if (detection.explanation?.per_model) {
                // 新格式：explanation.per_model.{model}.top_tokens
                const allTokens: Array<any> = []
                for (const [modelKey, modelData] of Object.entries(detection.explanation.per_model)) {
                    if (modelData?.top_tokens) {
                        allTokens.push(...modelData.top_tokens)
                    }
                }
                // 去重并取前20个
                const seen = new Set()
                return allTokens
                    .filter(t => t.score > 0 && !seen.has(t.piece) && seen.add(t.piece))
                    .slice(0, 20)
                    .map(item => ({
                        name: item.piece || item.token,
                        score: Math.abs(item.score),
                        description: `贡献分数 ${item.score.toFixed(4)}`
                    }))
            }
