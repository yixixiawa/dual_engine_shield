            ...
            <div class="result-summary">
                ...
                <div class="summary-item">
                    ...
                    <div class="summary-detail">
                        {{ ((result?.comprehensive_risk?.score || 0) * 100).toFixed(2) }}%
                    </div>
                ...
                <div class="summary-item">
                    ...
                    <div class="summary-value">
                        {{ Math.round((result?.detection_time_ms || 0) / 1000) }}s
                    </div>
                    <div class="summary-detail">
                        钓鱼 + 代码漏洞联合检测
                    </div>
            ...
            <div v-if="totalVulns > 0" class="vuln-details section-block">
                ...
                <div class="detail-grid">
                    <div class="detail-item">
                        <span class="detail-label">JavaScript 代码</span>
                        <span class="detail-value" :class="jsVulnCount > 0 ? 'text-danger' : 'text-success'">
                            {{ jsVulnCount > 0 ? `${jsVulnCount} 个` : '安全' }}
                        </span>
                        <div class="detail-sub">检测 {{ jsSnippetsCount }} 个代码块</div>
                    </div>
                    <div class="detail-item">
                        <span class="detail-label">PHP 代码</span>
                        <span class="detail-value" :class="phpVulnCount > 0 ? 'text-danger' : 'text-success'">
                            {{ phpVulnCount > 0 ? `${phpVulnCount} 个` : '安全' }}
                        </span>
                        <div class="detail-sub">检测 {{ phpSnippetsCount }} 个代码块</div>
                    </div>
                    <div class="detail-item">
                        <span class="detail-label">总计漏洞</span>
                        <span class="detail-value" :class="totalVulns > 0 ? 'text-danger' : 'text-success'">
                            {{ totalVulns > 0 ? `${totalVulns} 个` : '安全' }}
                        </span>
                    </div>
                </div>
...
const jsVulnCount = computed(() => {
    return result.value?.code_analysis?.javascript?.vulnerabilities?.length || 0
})

const jsSnippetsCount = computed(() => {
    return result.value?.code_analysis?.javascript?.snippets_count || 0
})

const phpVulnCount = computed(() => {
    return result.value?.code_analysis?.php?.vulnerabilities?.length || 0
})

const phpSnippetsCount = computed(() => {
    return result.value?.code_analysis?.php?.snippets_count || 0
})

const urlVulnCount = computed(() => 0)
const webVulnCount = computed(() => 0)
...
const overallRiskDesc = computed(() => {
    const level = overallRiskLevel.value
    if (level === 'critical') return '检测到极严重安全威胁，建议立即停止访问并处理'
...
const generateReport = () => {
    ...
    try {
        ...
        const report = {
            ...
            code_analysis: reportData.value.code_analysis,
            ...
            detection_time_ms: reportData.value.detection_time_ms
        }
...
.combined-detection {
    ...
    .overall-assessment {
        ...
        &.risk-high {
            ...
            border-color: rgba(255, 121, 63, 0.28);
        }
