            ...
            <div class="feature-list">
                <div v-for="(feature, idx) in features" :key="idx" class="feature-item">
                    <div class="feature-header">
                        ...
                        <span class="feature-score" :class="getFeatureScoreClass(feature.score)">
...
const scorePercent = computed(() => {
    return parseFloat((finalScore.value * 100).toFixed(1))
})
...
const features = computed(() => {
    ...
    // 1. 添加 GTE 模型的 token 归因特征（如果有）
    if (result.features && Array.isArray(result.features)) {
        featureList.push(...result.features)
    }
    
    // 2. 添加规则检查发现的风险因素（合并到特征列表，不显示来源）
    if (result.rule_check && result.rule_check.reasons && Array.isArray(result.rule_check.reasons)) {
        const ruleReasons = result.rule_check.reasons
        const ruleRiskScore = result.rule_check.risk_score || 0
        
        // 将每个规则原因转换为特征格式
        ruleReasons.forEach((reason: string, idx: number) => {
            featureList.push({
                name: reason,
                score: ruleRiskScore / ruleReasons.length, // 平均分配规则风险分数
                description: null
            })
        })
    }
...
// 根据最终风险等级和特征分数决定颜色
const getFeatureScoreClass = (score: number) => {
    // 如果整体是高风险，所有特征都显示为危险色
    if (finalScore.value >= 0.8) {
        return 'text-danger'
    }
    // 中风险：大于 0.5 的特征显示为警告色
    if (finalScore.value >= 0.5) {
        return score > 0.5 ? 'text-danger' : 'text-warning'
    }
    // 低风险或安全：正常判断
    return score > 0.5 ? 'text-danger' : 'text-success'
}
