...
class PhishingAnalysisService:
    ...
    def analyze(
        ...
        enable_rule_check: bool = True,  # 新增：是否启用规则检查
        rule_weight: float = 0.3,  # 新增：规则检查权重
    ) -> Dict[str, Any]:
        """
        执行钓鱼检测分析
        ...
        if rule_check_result and gte_score is not None:
            ...
            # 融合公式：final = (1 - rule_weight) * gte + rule_weight * rule
        ...
        result = {
            ...
            "explanation": None,
            "rule_check": prediction.get("rule_check"),  # 添加规则检查结果
        }
