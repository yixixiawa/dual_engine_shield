...
class CodeVulnerabilityDetectView(views.APIView):
            ...
            return Response(
                {
                    ...
                    'language': language,
                    # 为了前端兼容，将第一个漏洞的字段也放在顶层
                    **({} if not vulnerabilities else {
                        'cwe_id': vulnerabilities[0].get('cwe_id', ''),
                        'cwe_name': vulnerabilities[0].get('cwe_name', ''),
                        'vulnerability_type': vulnerabilities[0].get('vulnerability_type', ''),
                        'severity': vulnerabilities[0].get('severity', 'medium'),
                        'confidence': vulnerabilities[0].get('confidence', 0.0),
                        'description': vulnerabilities[0].get('description', ''),
                        'explanation': vulnerabilities[0].get('explanation', ''),
                        'fix_suggestion': vulnerabilities[0].get('fix_suggestion', ''),
                        'remediation': vulnerabilities[0].get('remediation', ''),
                    })
                },
    ...
    def _format_detector_result(result):
        ...
        if hasattr(result, 'to_dict'):
            ...
            return [{
                ...
                'cwe_id': result_dict.get('cwe_id', ''),
                'cwe_name': result_dict.get('cwe_name', ''),
                'vulnerability_type': result_dict.get('cwe_name', ''),
                'severity': result_dict.get('severity', 'medium'),
                'confidence': result_dict.get('confidence', 0.0),
                'description': result_dict.get('explanation', ''),
                'explanation': result_dict.get('explanation', ''),
                'remediation': result_dict.get('fix_suggestion', ''),
                'fix_suggestion': result_dict.get('fix_suggestion', ''),
                'line_number': result_dict.get('line_number', 0),
                'inference_time': result_dict.get('inference_time', 0),
                'input_tokens': result_dict.get('input_tokens', 0),
                'code': result_dict.get('code', {}).get('code', ''),
            }]
    ...
    @staticmethod
    def _format_detector_result_for_batch(result):
        """专为批量检测设计的格式化方法"""
        if result is None:
            return []
        
        # 如果是 VulnerabilityResult 对象
        if hasattr(result, 'to_dict'):
            result_dict = result.to_dict()
            return [{
                'is_vulnerable': result_dict['is_vulnerable'],
                    ...
                    if detector:
                        for snippet in code_snippets:
                            ...
                            try:
                                result = detector.detect(code, language)
                                # 复用统一的格式化方法
                                vulns = self._format_detector_result_for_batch(result)
                        ...
                        vuln_list = [
                            {
                                ...
                                'cwe_name': v.cwe_name if hasattr(v, 'cwe_name') else '',
                                'vulnerability_type': v.cwe_name if hasattr(v, 'cwe_name') else '',
                                'severity': (
                                    v.severity.value
                                ...
                                'explanation': v.description if hasattr(v, 'description') else '',
                                'fix_suggestion': v.fix_suggestion if hasattr(v, 'fix_suggestion') else '',
                                'remediation': v.fix_suggestion if hasattr(v, 'fix_suggestion') else '',
...
class UnloadModelView(views.APIView):
    """卸载 VR 模型释放 GPU 显存"""
    
    def post(self, request):
        """卸载模型"""
        try:
            logger.info("🔄 接收到卸载 VR 模型请求")
            
            unloaded = unload_vulnerability_model()
            
            if unloaded:
                return Response(
                    {
                        'success': True,
                        'message': 'VR 模型已卸载，GPU 显存已释放'
                    },
                    status=status.HTTP_200_OK
                )
            else:
    ...
    def get(self, request):
        """查询模型加载状态"""
        try:
            from .base import DETECTOR_INSTANCE, SCANNER_INSTANCE
            
            detector_loaded = DETECTOR_INSTANCE is not None and hasattr(DETECTOR_INSTANCE, '_is_loaded') and DETECTOR_INSTANCE._is_loaded
            scanner_loaded = SCANNER_INSTANCE is not None
            ...
            return Response(
                {
                    ...
                    'scanner_loaded': scanner_loaded,
                    'gpu_info': gpu_info
                },
                status=status.HTTP_200_OK
            )
        except Exception as e:
            logger.error(f"❌ 查询模型状态失败: {e}")
            return Response(
                {'error': str(e)},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR
            )
    ...
    def post(self, request):
        ...
        try:
            ...
            try:
                ...
                logger.info(f"✅ 创建综合检测日志记录，ID: {detection_log.id}")
            except Exception as e:
                logger.warning(f"⚠️  创建检测日志失败: {e}")
            ...
            # 步骤3: 代码提取与漏洞检测
            from api.coding_detect.extractors.html import HTMLCodeExtractor
            
            code_analysis = {
                'javascript': {'snippets_count': 0, 'vulnerabilities': []},
                'php': {'snippets_count': 0, 'vulnerabilities': []},
            }
            ...
            try:
                if CODING_DETECT_AVAILABLE:
                    ...
                    if detector and extractor:
                        # 提取代码片段
                        extracted_codes = extractor.extract_functions(html_content, url)
                        logger.info(f"从网页提取了 {len(extracted_codes)} 个代码片段")
                        
                        # 分别检测 JavaScript 和 PHP 代码
                        for extracted in extracted_codes:
                            lang = extracted.location.language
                            code = extracted.code
                            
                            if lang not in ['javascript', 'php']:
                                continue
                            
                            # 执行漏洞检测
                            result = detector.detect(code, lang)
                            vulnerabilities = self._format_detector_result(result)
                            
                            # 归类存储
                            if lang in code_analysis:
                                code_analysis[lang]['snippets_count'] += 1
                                code_analysis[lang]['vulnerabilities'].extend(vulnerabilities)
                        
                        logger.info(f"代码漏洞检测完成: JS={len(code_analysis['javascript']['vulnerabilities'])}, "
                                   f"PHP={len(code_analysis['php']['vulnerabilities'])}")
                    else:
                        logger.warning("⚠️  检测器或提取器未初始化")
                else:
                    logger.warning("⚠️  coding_detect 模块不可用")
            except Exception as e:
            ...
            code_vulnerabilities = (
                code_analysis['javascript']['vulnerabilities'] + 
                code_analysis['php']['vulnerabilities']
            )
            ...
            response_data = {
                ...
                'detection_time_ms': round(inference_time * 1000, 2)
            }
            ...
            if detection_log:
                try:
                    ...
                    detection_log.processing_time = inference_time
                    detection_log.save()
                    logger.info(f"✅ 综合检测结果已保存到数据库，日志ID: {detection_log.id}")
                except Exception as e:
