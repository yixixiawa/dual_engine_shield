...
class PHPCodeExtractor(BaseCodeExtractor):
    """PHP代码提取器"""

    FUNCTION_PATTERN = re.compile(
        r'^(?:public\s+|protected\s+|private\s+|static\s+|final\s+|abstract\s+)*function\s+(\w+)\s*\([^)]*\)\s*\{',
        re.MULTILINE,
    )
    ...
    def extract_functions(self, source_code: str, file_path: str = '') -> List[ExtractedCode]:
        ...
        logger.debug(f'从 {file_path} 提取了 {len(extracted)} 个PHP函数')
        return extracted
    ...
    def extract_from_file(self, file_path: str) -> List[ExtractedCode]:
        try:
            ...
            logger.error(f'读取PHP文件失败 {file_path}: {e}')
            return []
